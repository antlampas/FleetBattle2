    ...
    bool shipsOverlap(ShipPosition ship1,ShipPosition ship2)
    {
        ...
        if(isHorizontal(ship1))
            for(int column=ship1.first.second;column<=ship1.second.second;column++)
                ship1Squares.push_back(std::pair<int,int>(ship1.first.first,column));
        if(isVertical(ship1))
            for(int row=ship1.first.first;row<=ship1.second.first;row++)
                ship1Squares.push_back(std::pair<int,int>(row,ship1.first.second));
        if(isHorizontal(ship2))
            for(int column=ship2.first.second;column<=ship2.second.second;column++)
                ship2Squares.push_back(std::pair<int,int>(ship2.first.first,column));
        if(isVertical(ship2))
            for(int row=ship2.first.first;row<=ship2.second.first;row++)
                ship2Squares.push_back(std::pair<int,int>(row,ship2.first.second));
        if(std::find_first_of(ship1Squares.begin(),ship1Squares.end(),ship1Squares.begin(),ship1Squares.end()) != ship2Squares.end())
            return true;
        else
            return false;
    }
